...
export default function PendingClientsList() {
  const [list,   setList]   = useState([])
  const [search, setSearch] = useState('')

  useEffect(() => {
    axios.get(`${API}/api/eerettu/pending-clients`)
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <p className="text-center py-20 text-gray-400">No pending clients</p>
        )}
        {filtered.map(c => (
          <Link
            key={c.clientName}
          ...
          >
            ...
            <span className="bg-yellow-400 text-black text-sm font-bold px-3 py-1 rounded-xl">
